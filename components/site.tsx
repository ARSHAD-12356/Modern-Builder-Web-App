...
import { ArrowRight, Award, Baby, Bath, Building2, CalendarDays, CarFront, Check, ChevronDown, ChevronLeft, ChevronRight, Compass, Download, Dumbbell, Home, KeyRound, Leaf, Mail, MapPin, Maximize2, Menu, PanelTop, Phone, Play, Rotate3d, ShieldCheck, Sofa, Sprout, Star, Sun, Tag, TreePine, Trophy, TrendingUp, GraduationCap, Hospital, ShoppingCart, Users, Utensils, Waves, Wine, X } from 'lucide-react'
...
export function Header() {
  ...
  return <>
    <div className="utility-bar"><div className="utility-inner"><span><MapPin size={13}/> Patna, Bihar</span><span><Phone size={13}/> +91 98765 43210</span><span><Mail size={13}/> info@bshitech.com</span></div><div className="utility-inner utility-right"><span>Follow Us :</span><a className="utility-social facebook" href="#" aria-label="Facebook" title="Facebook">f</a><a className="utility-social instagram" href="#" aria-label="Instagram" title="Instagram">◎</a><a className="utility-social linkedin" href="#" aria-label="LinkedIn" title="LinkedIn">in</a><a className="utility-social youtube" href="#" aria-label="YouTube" title="YouTube">▶</a><button type="button" className="visit-top" onClick={() => scrollTo('contact')}><CalendarDays size={14}/> Book a Site Visit <ArrowRight size={14}/></button></div></div>
    <header className="main-nav"><button className="brand" type="button" onClick={() => scrollTo('top')}><img className="brand-logo" src={logoImage} alt="Bigrahpuram Developers Pvt. Ltd."/><span><strong>BIGRAHPURAM <b>DEVELOPERS</b></strong><small>BUILDING BETTER TOMORROW</small></span></button><button className="mobile-menu" aria-label="Toggle navigation" onClick={() => setOpen(!open)}>{open ? <X/> : <Menu/>}</button><nav className={open ? 'nav-links open' : 'nav-links'}>{nav.map(([label, id]) => <a href={`#${id}`} key={label} className={active === id ? 'active' : ''} onClick={(event) => { event.preventDefault(); scrollTo(id) }}>{label}</a>)}<div className="language"><span>English</span><span>हिंदी</span></div></nav></header>
  </>
}
...
export function Hero() { const [active,setActive]=useState(0); const titles=[<>A Brighter<br/><em>Tomorrow</em> in Patna</>,<>Vastu-Compliant<br/><em>Apartments</em></>,<>60% Green Area<br/><em>World-Class</em> Amenities</>]; return <section id="top" className="hero"><div className="hero-image" style={{backgroundImage:`url(${heroImage})`}}/><div className="hero-wash"/><div className="hero-content" key={active}><div className="hero-badge"><Sprout size={19}/> RERA Approved&nbsp; | &nbsp;60% Green Area&nbsp; | &nbsp;Starting at ₹5500/sqft</div><h1>{titles[active]}</h1><p className="hero-subtitle">Premium homes. Modern lifestyle. Lasting value.</p><div className="hero-features"><span><Home/><small>Spacious<br/>Apartments</small></span><span><Sprout/><small>Green & Open<br/>Surroundings</small></span><span><MapPin/><small>Prime<br/>Location</small></span><span><ShieldCheck/><small>RERA<br/>Approved</small></span></div><div className="hero-actions"><a className="gold-button" href="/contact"><CalendarDays size={18}/> Book a Site Visit <ArrowRight size={17}/></a><a className="outline-button" href="/floor-plans">▤ &nbsp; Download Brochure</a></div><div className="hero-stats"><span><strong>15+</strong><small>Years of Trust</small></span><span><strong>4</strong><small>Acres</small></span><span><strong>4</strong><small>Towers</small></span><span><strong>60%</strong><small>Green Area</small></span></div></div><div className="slide-list">{['Luxury Living','Modern Design','Green Living'].map((item,i)=><button key={item} className={i===active?'selected':''} onClick={()=>setActive(i)}><strong>0{i+1}</strong><span>{item}</span></button>)}</div><div className="hero-arrows"><button aria-label="Previous" onClick={()=>setActive((active+2)%3)}><ChevronLeft/></button><button aria-label="Next" onClick={()=>setActive((active+1)%3)}><ChevronRight/></button></div><div className="scroll-cue"><span>!</span><small>Scroll to Explore</small></div><div className="hero-ribbon"><span>◆</span><em>Building Communities<br/>Creating Brighter Futures</em></div><div className="video-card"><div className="play"><Play size={18} fill="currentColor"/></div><span>Watch<br/><strong>Project Video</strong></span></div></section> }
